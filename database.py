...
def add_guest(vk_id, name):
    ...
    cursor.execute(
        "INSERT INTO guests (vk_id, name, created_at, updated_at, registration_step, last_activity, visits_in_cycle, free_visit_available) VALUES (?,?,?,?,?,?,?,?)",
...
def update_guest(vk_id, **kwargs):
    ...
    cursor.execute(f"UPDATE guests SET {set_clause}, updated_at=? WHERE vk_id=?", values + [datetime.now().isoformat()])
    conn.commit()
...
def increment_cycle(vk_id):
    ...
    if row:
        ...
        if new_val >= FREE_HOOKAH_VISITS:
            cursor.execute("UPDATE guests SET visits_in_cycle=?, free_visit_available=1 WHERE vk_id=?", (FREE_HOOKAH_VISITS, vk_id))
            conn.commit()
            return True
        else:
