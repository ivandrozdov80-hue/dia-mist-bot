            ...
            for event in longpoll.listen():
                if event.type == VkEventType.MESSAGE_NEW and event.to_me:
                    try:
                        ...
                        # ============================================================
                        # 9. ЕСЛИ НЕТ ДАТЫ РОЖДЕНИЯ — ОБРАБАТЫВАЕМ ВВОД ДАТЫ
                        # ============================================================
                        if not has_birth and reg_step < 3:
                            ...
                            if re.match(r'^\d{1,2}\.\d{1,2}\.(?:\d{4}|\d{2})$', message):
                                ...
                                continue
                            # Проверяем, не хочет ли гость пропустить
                                ...
                                # Если это не дата и не 'пропустить', отправляем запрос
                                send_func(
                                    user_id,
                                    "📅 **УКАЖИ ДАТУ РОЖДЕНИЯ**\n"
                                    "━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                        ...
                        # ============================================================
                        # 10. ЕСЛИ НЕТ ДАТЫ, НО reg_step УЖЕ 3 (гость пропустил) — можно не трогать
                        # ============================================================
                        # (этот блок не нужен, так как выше уже обработали)
