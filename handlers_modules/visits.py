...
def handle_visit_button(vk, user_id, guest, send_func):
    """Обработка нажатия кнопки '✅ Визит'."""
...
def handle_visit_request(vk, user_id, guest, message, send_func):
    ...
    if reg_step < 3:
        send_func(
            user_id,
            "Сначала зарегистрируйся! Напиши /help, чтобы начать.",
    ...
    if last_request:
        try:
            ...
            if (now - last_time).total_seconds() < VISIT_REQUEST_COOLDOWN:
                wait_minutes = VISIT_REQUEST_COOLDOWN // 60
                send_func(
                    user_id,
                    f"Ты уже отправлял заявку недавно. Подожди {wait_minutes} минут.",
                    keyboard=kb.get_main_keyboard()
                )
                return True
        except Exception:
...
def handle_admin_reject(vk, user_id, message, send_func):
    """Отклонение заявки на визит администратором."""
    if user_id not in ADMIN_IDS:
        send_func(user_id, "⛔ Только для администраторов.")
        return True
